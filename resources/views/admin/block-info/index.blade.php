...
@section('title', trans('admin.block-info.actions.index'))

@section('body')

    <block-info-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/block-infos') }}'"
        inline-template>
                    ...
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.block-info.columns.id') }}</th>
                                        <th is='sortable' :column="'head_title1'">{{ trans('admin.block-info.columns.head_title1') }}</th>
                                        @if ($_GET['block_type'] == 'buildingA' || $_GET['block_type'] == 'buildingB' || $_GET['block_type'] == 'buildingC')
                                        <th is='sortable' :column="'block_name'">Code</th>
                                        @endif
                                        <th>Image</th>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <td>
                                            <div class="row no-gutters">
                                                <div class="col-auto">
                                                    <a class="btn btn-sm btn-spinner btn-info" :href="item.resource_url + '/edit?block_type={{ $_GET['block_type'] }}' " title="{{ trans('brackets/admin-ui::admin.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                </div>
                            ...
                            <!-- <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/block-infos/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.block-info.actions.create') }}</a>
                            </div> -->
                        </div>
                    </div>
                </div>
